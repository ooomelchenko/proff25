            ...
            $('.changeButton').click(function(){
                if($(this).val()==="1"){
                    $.ajax({
                        url: "changeBidParams",
                        method: "POST",
                        data: {bidId: $(this).parent().parent().find('.idBid').text(),
...
<div id="bidDiv" class="view">
    ...
    <table id="bidtbl" border="light" style="background-color: sandybrown">
        <tr id="addBidTr" hidden="hidden">
            ...
            <td> <button id="buttAddBid">Підтвердити</button> </td>
        </tr>
        ...
        <%for(Bid bid: bidList){%>
        <tr style="background-color: #ffffe0" class="bidTr" align="center">
            <td class="idBid"><%=bid.getId()%></td>
            <td class="bidData"><input class="newBD" datatype="date" hidden="hidden" value="<%if(bid.getBidDate()!=null)out.print(sdf.format(bid.getBidDate()));%>"><b><%if(bid.getBidDate()!=null)out.print(sdf.format(bid.getBidDate()));%></b></td>
            <td class="bidData">
                <select class="ex" hidden="hidden">
                    <% for (Exchange ex : exchangeList) {%>
                    <option value="<%=ex.getId()%>" <%if(ex.getCompanyName().equals(bid.getExchange().getCompanyName()))out.print("selected=selected");%>>
                        <%out.print(ex.getCompanyName());%>
                    </option>
                    <%}%>
                </select>
                <b><%=bid.getExchange().getCompanyName()%></b>
            </td>
            <td class="bidData"><input class="newNP" type="text" hidden="hidden" value="<%if(bid.getNewspaper()!=null)out.print(bid.getNewspaper());%>"><b><%=bid.getNewspaper()%></b></td>
            <td class="bidData"><input class="newND1" datatype="date" hidden="hidden" value="<%if(bid.getNews1Date()!=null)out.print(sdf.format(bid.getNews1Date()));%>"> <b><%if(bid.getNews1Date()!=null)out.print(sdf.format(bid.getNews1Date()));%></b></td>
            <td class="bidData"><input class="newND2" datatype="date" hidden="hidden" value="<%if(bid.getNews2Date()!=null)out.print(sdf.format(bid.getNews2Date()));%>"> <b><%if(bid.getNews2Date()!=null)out.print(sdf.format(bid.getNews2Date()));%></b></td>
            <td class="bidData"><input class="newRED" datatype="date" hidden="hidden" value="<%if(bid.getRegistrEndDate()!=null)out.print(sdf.format(bid.getRegistrEndDate()));%>"> <b><%if(bid.getRegistrEndDate()!=null)out.print(sdf.format(bid.getRegistrEndDate()));%></b></td>
            <td ><button class="changeButton">Редагувати</button></td>
